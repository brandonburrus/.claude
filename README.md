...
The `orchestrate-delivery` skill walks this whole arc end to end for a substantial effort, invoking the right skill at each stage and stopping at the evidence gate between them; the individual skills still auto-load on their own for smaller work, and `review-metrics` closes the loop by measuring a launch against the spec's success metrics so the next cycle starts from evidence.
