...
- **Secrets hygiene**: `.env*` and key files in `.gitignore`, `.env.example` with placeholders committed; a secret that reaches a remote is rotated, not deleted (pr's staged-diff scan is the last line, not the plan)
