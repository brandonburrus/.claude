...
- **Check the working directory the baseline agents will inherit.** Parking is defeated if the agents wake up next to the parked file: an observed baseline read and quoted a parked SKILL.md because the orchestrator's shell had `cd`-ed into the skill directory before the `mv`, and the inherited cwd followed the moved inode into the park location. Before spawning baselines, `cd` to a neutral directory containing nothing skill-related, and park outside the directory tree the agents can trivially browse (a sibling directory of their cwd is discoverable in one `ls ..`). Parking and pointing with-skill agents at the parked path is still the preferred setup, since it lets baseline and with-skill runs execute in parallel
- **Baselines spawned right after parking can still see the skill in the registry.** The skill listing injected into a subagent can lag the filesystem: an observed baseline reported the just-parked skill's name appearing in its own system reminder, and another reproduced the skill's distinctive output format with no file access. After parking, treat any baseline that names the skill or mirrors its signature structures as contaminated; discard it and rerun once the registry has refreshed (a later spawn in the same session sufficed)
- **When the test prompt mutates a sandbox, give each run its own copy.** Parallel baseline and with-skill agents pointed at one shared repo corrupt each other: two observed runs each saw the other's edits as mysterious external file reverts, committed transient content, and burned effort reconciling state that the test harness, not the scenario, had broken. Clone the sandbox per agent (or serialize the runs) whenever agents will write
