...
- [ ] 6. Validate live behavior on runnable surfaces (manual-tester)
...
### 6. Validate live (runnable surfaces)

Green and reviewed is not the same as working: the suite and the reviewers exercise code paths and the diff, and neither renders the UI, hits the live endpoint, or runs the built artifact. When the change has a runnable surface, validate it after the pipeline is green. Conduct here too: dispatch the `manual-tester` agent with the change summary and the running app URL; it drives the surface and returns a pass/fail-with-evidence report. Fall back to running `validate-web` or `validate-api` directly in the main thread only when the `manual-tester` agent is unavailable.

| Surface | Route |
| ------- | ----- |
| Web UI | `manual-tester` against the running dev server (it wraps `validate-web`) |
| HTTP API | `manual-tester` against the running service (it wraps `validate-api`) |
| CLI / TUI / worker / desktop | Drive the built artifact directly: the run skill, or the project's own run instructions, observing the changed behavior live |
...
- **Green is not validated.** A verified, reviewed, green change can still render a broken UI or return 500 on a real request, because the suite proves the logic, not the running system. Live validation (the `manual-tester` agent wrapping `validate-web` / `validate-api`, or driving the built artifact directly) is the post-pipeline step that closes that gap; skip it only when there is no runnable surface, and say so.
...
7. Validate live: T2 is a new endpoint, so dispatch manual-tester against the running service
   (it runs validate-api); the saved request returns 201 and the asserted body shape holds.
   Collection saved to bruno/.
