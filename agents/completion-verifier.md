...
4. **Sweep for reward hacking.** A green suite is necessary, never sufficient. Check the diff and tests for: skipped or disabled tests (`test.skip`, `xit`, commented-out cases), assertions on constants or empty bodies, assertions loosened or deleted relative to the base, tests that never exercise the changed code, and requirements with no test at all. Each is evidence the green is decorative.
