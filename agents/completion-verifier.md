...
description: >-
  Use this agent to independently verify that claimed-complete work
...
4. **Sweep for reward hacking and coverage gaps.** A green suite is necessary, never sufficient. Check the diff and tests for: skipped or disabled tests (`test.skip`, `xit`, commented-out cases), assertions on constants or empty bodies, assertions loosened or deleted relative to the base, tests that never exercise the changed code, and requirements with no test at all. Each is evidence the green is decorative. Then check coverage shape: each feature should have a golden-path, an error-case, and an edge-case test (the canonical statement of this three-test floor lives in the follow-tdd skill); a feature exercised only on the happy path is a coverage gap worth reporting even when the suite is green, unless the implementer stated why a category does not apply. For a feature with a user-facing surface, also confirm the complete user journey is covered end to end (an e2e test, or a recorded live validation that walked the full flow a real user takes to accomplish the goal), not merely the units in isolation; a feature whose parts each pass but whose end-to-end flow was never exercised is a coverage gap, since a flow can break at a seam where every individual step passes.
